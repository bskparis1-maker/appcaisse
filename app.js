...
function fetchStockFromSheet() {
  ...
  window[callbackName] = function (data) {
    try {
      ...
      // Une fois les produits chargés, on peut charger les ventes
      fetchSalesFromSheet();
  ...
  script.onerror = function () {
    console.warn("Erreur lors du chargement du stock, utilisation des données locales.");
    renderProductsForSale();
    renderProductsTable();
    renderCart();
    fetchSalesFromSheet();
...
function sendClientToSheet(client) {
  const payload = encodeURIComponent(JSON.stringify({ client: client }));
...
function fetchClientsFromSheet() {
  ...
  script.onerror = function () {
    console.warn("Erreur lors du chargement des clients, utilisation des données locales.");
    renderClientSelect();
    renderClientsTable();
  };
  document.body.appendChild(script);
...
function fetchSalesFromSheet() {
      ...
      if (data && Array.isArray(data.sales)) {
        sales = data.sales.map(s => {
          ...
          return {
            ...
            loyaltyDiscount: Number(info.loyaltyDiscount) || 0,
  ...
  script.onerror = function () {
    console.warn("Erreur lors du chargement des ventes, utilisation des données locales.");
    renderSalesTable();
    updateTodayTotal();
    updateTodayTotalsByPayment();
    renderClientsTable();
...
// ================== CALCUL TOTAL PANIER (PROMO + REMISE) ==================
function calculateCartTotals() {
  ...
  // Promo : chaque paire de produits à 8000 => -1000
...
// ================== ENVOI VENTE -> GOOGLE SHEETS ==================
function sendSaleToSheet(sale) {
  const payload = encodeURIComponent(JSON.stringify({ sale: sale }));
...
function confirmSale() {
  ...
  // Client + fidélité par TRANCHE de 100 000 F
  const select = document.getElementById("client-select");
  let clientId = null;
  let clientName = "";
  let clientPhone = "";
  ...
  if (select && select.value) {
    ...
    if (client) {
      ...
      // Total déjà dépensé par ce client (basé sur toutes les ventes)
      let clientTotalBefore = 0;
      sales.forEach(s => {
        if (s.clientId === clientId) {
          clientTotalBefore += s.total || 0;
        }
      });

      const totalBeforeLoyalty = totals.finalTotal;

      // Nombre de tranches de 100 000 F avant et après cette vente
      ...
      // On applique -10% seulement si on franchit un nouveau palier
  ...
  // Mise à jour client côté local
  if (clientId) {
    const client = clients.find(c => c.id === clientId);
    if (client) {
      // on recalcule à partir de sales + cette vente
      let totalSpent = 0;
      sales.forEach(s => {
...
function openReceiptWindow(sale) {
  ...
  if (!win) {
    alert("Veuillez autoriser les fenêtres pop-up pour afficher le ticket.");
    return;
  }
  win.document.open();
...
function renderClientsTable() {
  ...
  list.forEach(c => {
    // Recalcul rapide du totalSpent si pas défini
...
function openDailyReport() {
  ...
  todaySales.forEach(sale => {
    ...
    sale.items.forEach(item => {
      ...
      const unitPrice = product ? product.price : 0;
      if (!productMap[name]) {
        productMap[name] = { qty: 0, amount: 0, unitPrice: unitPrice };
      }
  ...
  if (!win) {
    alert("Veuillez autoriser les fenêtres pop-up pour afficher le rapport.");
    return;
  }
  win.document.open();
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetchStockFromSheet();   // stock + ensuite ventes
